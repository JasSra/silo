...
public class AzureOpenAIMetadataService : IAIMetadataService
{
    private readonly HttpClient _httpClient;
    private readonly AzureOpenAIConfiguration _config;
    private readonly ILogger<AzureOpenAIMetadataService> _logger;

    public string ProviderName => "AzureOpenAI";

    public AzureOpenAIMetadataService(
        HttpClient httpClient,
        IOptions<AIConfiguration> aiConfig,
        ILogger<AzureOpenAIMetadataService> logger)
    {
        _httpClient = httpClient;
        _config = aiConfig.Value.AzureOpenAI ?? throw new ArgumentException("Azure OpenAI configuration is required");
        _logger = logger;
    ...
    {
        var ok = !string.IsNullOrWhiteSpace(_config.Endpoint) && !string.IsNullOrWhiteSpace(_config.ApiKey)
    ...
    public async Task<AIMetadataResult> ExtractMetadataAsync(AIMetadataRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Extracting metadata for file: {FileName} with Azure OpenAI", request.FileName);

            var isImageFile = IsImageFile(request.MimeType);
            var deployment = isImageFile ? _config.Deployment : _config.TextDeployment;
            ...
            var url = $"{_config.Endpoint.TrimEnd('/')}/openai/deployments/{deployment}/chat/completions?api-version={_config.ApiVersion}";
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return AIMetadataResult.Failed($"Azure OpenAI API error: {response.StatusCode}");
            }
            ...
            {
                _logger.LogError(ex, "Failed to parse Azure OpenAI response envelope");
            }

            if (string.IsNullOrWhiteSpace(contentStr))
                return AIMetadataResult.Failed("No response content from Azure OpenAI");

            var extracted = ParseMetadataResponse(contentStr!);
            return extracted;
        }
        catch (Exception ex)
        {
    ...
    private object CreateVisionMessage(AIMetadataRequest request)
    {
        ...
        return new
        {
            ...
            content = new object[]
            {
                new { type = "text", text = GetMetadataExtractionPrompt(request, true) },
    ...
    {
        return new { role = "user", content = GetMetadataExtractionPrompt(request, false) };
    }
